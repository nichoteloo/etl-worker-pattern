...
func ParseTransform(line []string) entities.Building {
	...
	// at the moment line[0] is used for index 0 - 12
	myData["BASE_BBL"] = sanitizeConv(line[1], "string")
	myData["MPLUTO_BBL"] = sanitizeConv(line[2], "string")
	myData["BIN"] = sanitizeConv(line[3], "float")
	myData["NAME"] = sanitizeConv(line[4], "string")
	myData["LSTMODDATE"] = sanitizeConv(line[5], "date")
	myData["LSTSTATTYPE"] = sanitizeConv(line[6], "string")
	myData["CNSTRCT_YR"] = sanitizeConv(line[7], "float")
	myData["DOITT_ID"] = sanitizeConv(line[8], "float")
	myData["HEIGHTROOF"] = sanitizeConv(line[9], "float")
	myData["FEAT_CODE"] = sanitizeConv(line[10], "float")
	myData["GROUNDELEV"] = sanitizeConv(line[11], "float")
	myData["GEOM_SOURCE"] = sanitizeConv(line[12], "string")
